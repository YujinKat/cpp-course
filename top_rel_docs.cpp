...
set <string> query = ParseQuery(raw_query, stop_words);
vector<pair<int, int>> AllRelevant = FindAllDocuments(documents, query);
sort(AllRelevant.begin(), AllRelevant.end());
    reverse(AllRelevant.begin(), AllRelevant.end());
vector<pair<int, int>> TopRelevant;
for (int i = 0; i < AllRelevant.size(); ++i) {
    if(AllRelevant[i].first != 0){
    TopRelevant.push_back(AllRelevant[i]);
    }
}
return TopRelevant;
}
...
int main() {  
    ...
    vector<pair<int, int>> TopDocs = FindTopDocuments(documents, stop_words, query);
    if (TopDocs.size() > MAX_RESULT_DOCUMENT_COUNT) {
        TopDocs.resize(MAX_RESULT_DOCUMENT_COUNT);
    }
    for (auto [relevance, document_id] : TopDocs) {
